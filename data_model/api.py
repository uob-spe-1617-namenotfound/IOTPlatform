...
device1 = model.device("user_id_1", "house_id_1","'room_id_1", "device_id_1", "Theromstat", "on")
devicegroup = model.DeviceGroupRepository()
devicegroup = model.DeviceGroups("'devicegroup_id_A")
@api.route('/user/<string:user_id>')
def get_user_info(user_id):
    user = user_repository.get_user_by_id(user_id)
    return jsonify({"user": user.get_user_attributes(), "error": None})

@api.route('/user/<string:house_id>')
